...
const std::string defaultPath = "./statistics/";
const double defaultEps = 0.001;

double mockComputeMean (double values[], vector<int> &scenario) {
  return CalculateMean(values, scenario[0]);
}

bool testComputeMean (string path, double eps, bool print) {
  cout << "Testing the function computeMean()...\t\t\t\t";
  ...
  cout << (result ? "[OK]" : "[NOK]") << endl;
  return result;
}

double mockComputeVariance (double values[], std::vector<int> &scenario) {
  return CalculateVariance(values, scenario[0]);
}

bool testComputeVariance (string path, double eps, bool print) {
  cout << "Testing the function computeVariance()...\t\t\t";
...
double mockComputeSampleVariance (double values[], vector<int> &scenario) 
{
  return CalculateSampleVariance(values, scenario[0]);
}

bool testComputeSampleVariance (string path, double eps, bool print) 
{
  cout << "Testing the function computeSampleVariance()...\t\t\t";
...
double mockComputeStandardDeviation (double values[], vector<int> &scenario)
{
  return GetStandardDeviation(values, scenario[0]);
}

bool testComputeStandardDeviation (string path, double eps, bool print) 
{
  cout << "Testing the function computeStandardDeviation()...\t\t";
...
double mockComputeSampleStandardDeviation (double values[], vector<int> &scenario) {
  return GetSampleStandardDeviation(values, scenario[0]);
}

bool testComputeSampleStandardDeviation (string path, double eps, bool print) {
  cout << "Testing the function computeSampleStandardDeviation()...\t";
  ...
  cout << (result ? "[OK]" : "[NOK]") << std::endl;
  return result;
}

void demoArrayStats (double sample[], int length)
{
    double mean = CalculateMean(sample, length);
    double variance = CalculateVariance(sample, length);
    double samplevariance = CalculateSampleVariance(sample, length);
    double sampledevi = GetSampleStandardDeviation(sample, length);
    double devi = GetStandardDeviation(sample, length);
